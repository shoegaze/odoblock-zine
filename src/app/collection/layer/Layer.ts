...
export const idToZPos = (id: number): number => -id * layersDistance
...
export const zPosToId = (zPos: number): number => Math.abs(Math.min(0, Math.floor(zPos / layersDistance)))

let idLayer = 0
export const createLayer = (...scenes: AppScene[]): Layer => ({
  id: idLayer++,
  zPos: idToZPos(idLayer - 1),
  scenes,
